...
const analyticsData = [
  ...
  },
  {
    ...
    chartData: [
      ...
      { name: 'Page C', uv: 2800 },
      { name: 'Page D', uv: 2400 },
      { name: 'Page E', uv: 3000 }, { name: 'Page F', uv: 2700 }, { name: 'Page G', uv: 3800 },
    ],
  },
];
    ...
    <div className="group rounded-2xl border border-white/10 bg-gray-800/40 p-5 shadow-lg transition-all duration-300 ease-in-out hover:border-white/20 hover:bg-gray-800/60 transform hover:-translate-y-1 cursor-pointer">
      ...
      <div className="mt-4 flex items-end justify-between">
        <div className="flex flex-col">
          ...
          <p className={`mt-1 text-xs ${changeType === 'positive' ? 'text-green-400' : 'text-red-400'}`}>
            {change}
          </p>
        </div>
  ...
  return (
    <div className="w-full mb-8">
      <div className="flex items-center justify-between pb-6 border-b border-gray-200 mb-8">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-gray-900">Dashboard de Performance</h1>
          <p className="mt-1 text-sm text-gray-500">Resumo de desempenho do mês atual.</p>
        </div>
        <button className="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow-sm transition-colors hover:bg-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500">
          Atualizar
        </button>
      </div>
